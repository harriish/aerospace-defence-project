...
async fn main() -> Result<()> {
    ...
    match args.protocol.as_str() {
        ...
        "udp" => run_encrypted_udp_test(&payload).await?,
...
async fn run_encrypted_udp_test(payload: &[u8]) -> Result<()> {
    ...
    let nonce_bytes = [5u8; 12]; // 12-byte IV initialization vector
    let nonce = Nonce::from_slice(&nonce_bytes);
    
    let mut encrypted_packet = cipher.encrypt(nonce, payload).map_err(|e| anyhow::anyhow!(e.to_string()))?;
    encrypted_packet.extend_from_slice(&nonce_bytes); // Append nonce to packet end

    let start_tx = Instant::now();
    socket.send(&encrypted_packet).await?;
    let mut buf = vec![0u8; 65536];
    
    let tx_duration = match tokio::time::timeout(Duration::from_millis(1500), socket.recv(&mut buf)).await {
        Ok(Ok(len)) => {
            // Decrypt return payload to balance processing workload overhead
            let incoming_data = &buf[..len];
            if incoming_data.len() > 12 {
                let (encrypted_reply, reply_nonce_bytes) = incoming_data.split_at(incoming_data.len() - 12);
                let reply_nonce = Nonce::from_slice(reply_nonce_bytes);
                let _decrypted = cipher.decrypt(reply_nonce, encrypted_reply).map_err(|e| anyhow::anyhow!(e.to_string()))?;
            }
            start_tx.elapsed()
        },
